...
// @features		可以自动展开字数超长的微博内容；增加对首页“好友赞过的微博”的屏蔽；增加对首页热门微博（粉丝头条）的屏蔽；增加对“让红包飞”标识的屏蔽；修正个人主页自动跳转全部微博功能失效的问题
// @version			2.2.3.1
// @revision		104
// @author			@富平侯
...
var $dialog = (function () {
	...
	var addUsers = function (id, list) {
		...
		users.forEach(function (user) {
			...
			STK.jsonp({
				url : 'http://' + document.domain + '/aj/v6/user/newcard?ajwvr=6',
				args : request,
				method : 'get',
				responseType : 'json',
				varkey : 'callback',
...
var $filter = (function () {
	...
	var apply = function (feed) {
		...
		if ($options.filterPaused || // 暂停屏蔽
			...
			// 屏蔽好友赞过的微博
			if (scope === 1 && $options.filterLiked && feed.querySelector('.WB_cardtitle_b .ficon_praised')) {
				return true;
			}
	...
	document.addEventListener('click', function (event) {
		...
		if (actionType === 'widget_commentPhotoView' && actionData && actionData.match(/pid=(\w+)&cid=(\d+)/)) {
			// 评论图片
			window.open('http://ww4.sinaimg.cn/large/' + RegExp.$1 + '.jpg', '_blank');
			event.stopPropagation();
...
var $page = (function () {
	...
	var redirectToAllFeeds = (function () {
			var blockable, blocked; // 人为点击“热门”时闭锁本功能
			var checkRedirect = function () {
				if (blocked) {
					// 人为点击
					return;
				} else if (location.href.match(/\Wis_([a-z]+)=1\W?/)) {
					// href中出现is_hot标志时跳转才能成功
					if (RegExp.$1 === 'hot') {
						var allFeedsBtn = document.querySelector('li[node-type="tab_all"]');
						if (allFeedsBtn) {
							allFeedsBtn.click();
							return;
						}
			...
			return function () {
				...
				var hotFeedsBtn = document.querySelector('li[node-type="tab_hot"]');
				if (!blockable && hotFeedsBtn) {
					hotFeedsBtn.addEventListener('click', function () {
							blocked = true;
						}, true); // 使用事件捕捉以尽早触发事件，闭锁本功能
					blockable = true;
				}
				checkRedirect();
			};
	...
	var customStyles = function () {
		// 修正会员微博背景导致紧凑型工具栏无法点击的问题
		var cssText = '.WB_feed_vipcover .WB_feed_detail { position: static !important }\n',
...
$.get($.uid.toString(), undefined, function (options) {
	...
	if (!$options.load(options)) {
		...
		if ($options.updateNotify) {
			alert('您已更新到“眼不见心不烦”v2.2.3：\n\n- ' + '可以自动展开字数超长的微博内容；增加对首页“好友赞过的微博”的屏蔽；增加对首页热门微博（粉丝头条）的屏蔽；增加对“让红包飞”标识的屏蔽；修正个人主页自动跳转全部微博功能失效的问题'.split('；').join('\n- '));
		}
	}
	init();
});
